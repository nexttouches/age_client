...
package starling.textures
{
	...
	{
		/**
		 * 全部 ConcreteTexture 的显存占用
		 */
		public static var totalVRam:uint;

		/**
		 * 显存占用
		 */
		public var vram:uint;
		...
		public function uploadBitmapData(data:BitmapData):void
		{
			...
			if (data.width != mWidth || data.height != mHeight)
			{
				...
				data = potData;
				vram += potData.width * potData.height * 4;
			}
			...
			if (mBase is flash.display3D.textures.Texture)
			{
				...
				if (mMipMapping && data.width > 1 && data.height > 1)
				{
					...
					while (currentWidth >= 1 || currentHeight >= 1)
					{
						...
						vram += canvas.width * canvas.height * 4;
					}
					canvas.dispose();
				}
			}
		...
		starling_internal function createBase():void
		{
			...
			var isPot:Boolean = mWidth == getNextPowerOfTwo(mWidth) && mHeight == getNextPowerOfTwo(mHeight);
