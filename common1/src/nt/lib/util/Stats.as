...
package nt.lib.util
{
	...
	public class Stats extends Sprite
	{
		...
		public function Stats():void
		{
			...
			style.setStyle('xml', { fontFamily: 'Courier', leading: '-1px' });
			...
			text.multiline = true;
			text.wordWrap = true;
			rectangle = new Rectangle(WIDTH - 1, 0, 1, HEIGHT - text.height);
		...
		private function init(e:Event):void
		{
			...
			graphics.endFill();
			addChild(text);
			graph = new BitmapData(WIDTH, HEIGHT - text.height, false, colors.bg);
			graphics.beginBitmapFill(graph, new Matrix(1, 0, 0, 1, 0, text.height));
			graphics.drawRect(0, 50, WIDTH, HEIGHT - text.height);
		...
		private function update(e:Event):void
		{
			...
			if (timer - 1000 > ms_prev)
			{
				...
				mem_graph = Math.min(graph.height, Math.sqrt(Math.sqrt(mem * text.height * 100))) - 2;
				mem_max_graph = Math.min(graph.height, Math.sqrt(Math.sqrt(mem_max * text.height * 100))) - 2;
				graph.scroll(-1, 0);
